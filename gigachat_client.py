...
def get_gigachat_response(user_message, tasks_context):
    ...
    try:
        ...
        # Формируем запрос
        full_prompt = f"""Ты — добрый и поддерживающий ассистент по продуктивности. Отвечай мягко, с теплом и пониманием. Используй слова поддержки: 'ты молодец', 'давай вместе', 'у тебя всё получится'. Не критикуй, а мягко направляй.
